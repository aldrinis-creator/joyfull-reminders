  ...
  return (
    <section className="bg-card shadow-card space-y-4 rounded-3xl p-5">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-xl">Sync to your calendar</h2>
      </div>
      <p className="text-muted-foreground text-sm">
        Add this as a subscribed calendar in Google, Apple or Outlook — your reminders show up
        there and stay in sync automatically.
      </p>
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          <Button
            className="h-12"
            disabled={!token}
            onClick={() => void copy(webcalUrl, "Calendar link")}
          >
            <Copy className="size-4" aria-hidden /> Copy calendar link
          </Button>
          <Button
            variant="outline"
            className="h-12"
            disabled={!token}
            onClick={() => void copy(httpsUrl, "Web link")}
          >
            <Copy className="size-4" aria-hidden /> Copy https link
          </Button>
        </div>
      </div>

      <Button variant="ghost" className="h-12 w-full" disabled={busy} onClick={() => void rotate()}>
        <RefreshCw className="size-4" aria-hidden /> Get a new link
      </Button>
